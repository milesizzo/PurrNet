...
namespace PurrNet.Modules
{
    ...
    public class HierarchyV2
    {
        ...
        private void SetupSceneObjects(Scene scene)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                for (int j = 0; j < cc; j++)
                {
                    ...
                    child.PreparePrefabInfo(Guid.NewGuid(), child == first ? j : first.componentIndex, true, true);
        ...
        public void OnGameObjectCreated(GameObject obj, GameObject prefab)
        {
            ...
            if (!_manager.prefabProvider.TryGetPrefabData(prefab, out var prefabData))
                return;

            NetworkManager.SetupPrefabInfo(obj, prefabData);
